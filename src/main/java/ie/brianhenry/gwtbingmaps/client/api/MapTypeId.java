package ie.brianhenry.gwtbingmaps.client.api;

public class MapTypeId {

	/**
	 * The aerial map type is being used.
	 */
	public static String AERIAL = "aerial";
	
	/**
	 * The map is set to choose the best imagery for the current view.
	 */
	public static String AUTO = "auto";
	
	/**
	 * The bird's eye map type is being used.
	 */
	public static String BIRDSEYE = "birdseye";
	
	/**
	 * Collin's Bart (mkt=en-gb) map type is being used.
	 */
	public static String COLLINSBART = "collinsBart";
	
	/**
	 * The Mercator map type is being used. This setting removes the base map tiles.
	 */
	public static String MERCATOR = "mercator";
	
	/**
	 * Ordnance Survey (mkt=en-gb) map type is being used.
	 */
	public static String ORDNANCESURVEY = "ordnanceSurvey";
	
	/**
	 * The road map type is being used.
	 */
	public static String ROAD = "road";
	
	/**
	 * The Facebook map type is being used.
	 */
	public static String FACEBOOK = "fb";
}
